...
def call_anthropic(api_key: str, text: str) -> dict:
    schema = {
      "type":"object",
      "properties":{k:{
        "type":"object",
    ...
    try:
        ...
        # Fallback: try to extract JSON block if extra text slipped in
        start = out_text.find("{")
        end = out_text.rfind("}")
...
if not api_key:
    st.info("Add your ANTHROPIC_API_KEY in Streamlit Secrets.")
...
if st.button("Score"):
    if not text.strip():
        ...
        with st.spinner("Scoring…"):
            scores = call_anthropic(api_key, text)
