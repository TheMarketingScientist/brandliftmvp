...
BRAND_NAVY   = "#2E3C71"
COMP_TEAL    = "#2AA9A1"  # Competitor (new)
...
COMP_PHRASES = {
    "Leadership": [
        "market-leading claims", "award-winning reputation",
        "trend-setting positioning", "recognized authority"
    ],
    "Ease_of_Use": [
        "simple onboarding cues", "clear action verbs",
        "one-step instructions", "low-friction setup"
    ],
    "Quality": [
        "durability emphasis", "precision language",
        "rigorous testing mention", "craftsmanship cues"
    ],
    "Luxury": [
        "exclusive tone", "premium finish cues",
        "elevated aesthetic", "boutique language"
    ],
    "Cost_Benefit": [
        "value-centric framing", "ROI emphasis",
        "savings mention", "high benefit per cost"
    ],
}
...
if base_scores or improved_scores or comp_scores:
    ...
    # Determine visible columns count
    cols = [1 if base_scores else 0, 1 if improved_scores else 0, 1 if comp_scores else 0]
    count = sum(cols) or 1
    layout = st.columns(count)
...
# Delta table remains for Original vs Improved
