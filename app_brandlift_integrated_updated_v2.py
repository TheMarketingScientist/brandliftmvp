...
def rewrite_copy_to_targets(api_key: str, text: str, target_levels: dict) -> str:
    ...
    # Present [] in a compact JSON-like string for the model
    ...
    instr = f"""Rewrite the ad copy to better align perceived brand attributes with these [] (0–1):
{targets_str}
...
def rewrite_copy(api_key: str, text: str, []: list[str]) -> str:
    t = ", ".join([]) if [] else "Leadership"
    example = json.dumps({"rewrite": "..."}, ensure_ascii=False)
...
def propose_new_ideas(api_key: str, base_text: str, []: list[str], n: int = 2) -> list[str]:
    t = ", ".join([]) if [] else "Leadership"
...
with st.expander("🎯 Desired attribute []", expanded=True):
    desired_targets = {}
    cols = st.columns(3)
    for i, a in enumerate(ATTRS):
        with cols[i % 3]:
            pretty = _pretty_attr(a)
            # Default [] are moderate-strong to create a "wow" effect on first demo
            desired_targets[pretty] = st.slider(pretty, 0.0, 1.0, 0.78, 0.01, help="Target perceived level for this attribute")
    st.caption("These [] drive the improvement step. The rewrite will **avoid explicit attribute tags** and aim for these levels in tone and content.")



improve_clicked = st.button("Improve & Rescore", use_container_width=True, key="improve_and_rescore")
if score_clicked:
    if not api_key:
...
if improve_clicked:
    ...
    if not api_key:
        ...
        # Build numeric [] from sliders
        # desired_targets has pretty labels; convert inside helper
        with st.spinner("Generating improved copy to match []…"):
