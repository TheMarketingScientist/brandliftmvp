...
def render_correlation_section():
    ...
    if df_long is None or df_long.empty or not set(["entity","channel","variant","attribute","score"]).issubset(set(df_long.columns)):
...
def _seed_demo_trends():
    ...
    if "monthly_attr_trends" in st.session_state:
        ...
        if not set(["Entity","Variant"]).issubset(df.columns):
            ...
            return
    _init_records()
    # Only seed demo trends if we either have real records or the test-user preload is enabled
    if not st.session_state["score_records"]:
        if st.session_state.get('preload_demo'):
            _seed_full_demo_heatmap()
        else:
            # No records and not a test user → do not create synthetic trends
            return
...
if trend_df is None or trend_df.empty:
    ...
    # Stop the trends section early so no downstream code runs
    trend_df = None
