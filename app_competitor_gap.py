...
def inject_css():
    ...
    /* Trailing highlight */
    .warn-box {{
        background: {AMBER_BG}; border: 1px solid {AMBER_BORDER}; border-radius: 8px;
    ...
    .pill-warn {{
        ...
        background: white; border: 1px solid {AMBER_BORDER}; color: {BRAND_NAVY};
...
for k in ATTRS:
    ...
    name = k.replace("_"," ")
    badges.append(f'<span class="badge" style="background:{c}">{name}</span>')
...
if improved_scores and comp_scores:
    ...
    if html:
        ...
        # Suggest generating new ideas targeting top trailing attributes
