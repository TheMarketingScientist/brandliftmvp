...
def random_competitor_scores(seed: int | None = None) -> dict:
    ...
    for k in ATTRS:
        ...
        phrase = random.choice(COMP_PHRASES[k if k in COMP_PHRASES else "Leadership"])
...
def _records_to_channel_attr_medians(entities: list[str] | None = None, variants: list[str] | None = None) -> pd.DataFrame:
    """Return a pivot table with channels as rows and attributes as columns, values = median scores across filtered records."""
    _init_records()
    rows = []
    for r in st.session_state["score_records"]:
...
# Trailing attributes vs competitor (use a plain string style block to avoid f-string braces)
...
# ------- Pre-populate full demo data for Heatmap (all channels) -------
def _seed_full_demo_heatmap():
    _init_records()
    if st.session_state["score_records"]:
        return  # don't overwrite an active session

    # Deterministic seeds per channel/variant so your demo looks the same every run
    for i, ch in enumerate(CHANNELS):
        # Client - Original
        s1 = random_competitor_scores(seed=100 + i)
        _upsert_record("Client", ch, "Original", s1)

        # Client - Improved
        s2 = random_competitor_scores(seed=200 + i)
        _upsert_record("Client", ch, "Improved", s2)

        # Competitor A
        s3 = random_competitor_scores(seed=300 + i)
        _upsert_record("Competitor A", ch, "Competitor", s3)

# Call once on load (before building filters/heatmap_df)
_seed_full_demo_heatmap()

# Build dynamic filters from recorded items
